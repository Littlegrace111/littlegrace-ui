...
class MonthPicker extends Component {
	...
	handleClick = event => {
		...
		if (this.domNode.contains(event.target)) return;
	...
	// Ref 可以用来存储dom节点的引用，React会在组件挂载时给current属性传入DOM元素，并在组件卸载时传入null值。ref会在componentDidMount或componentDidUpdate生命周期钩子触发前更新。
	// Ref可以使用React.createRef() 创建，并通过ref属性附加到React元素上。访问Refs，当ref被传递给render中的元素，使用ref.current属性访问。
	// Ref 也支持回调函数的写法
	render() {
